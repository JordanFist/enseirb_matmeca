...
class TestPassagerIndecis extends TestPassagerAbstrait {
	...
	return new PassagerIndecis(nom, destination, new ArretNerveux());
    }
	...
	PassagerIndecis p = new PassagerIndecis("yyy", 5, new ArretNerveux());

	FauxVehicule faux = new FauxVehicule(FauxVehicule.VIDE);
	faux = new FauxVehicule(FauxVehicule.ASSIS);
	p.monterDans(faux);

	assert 0 == faux.logs.size() : "le passager ne veut pas s'assoir";
